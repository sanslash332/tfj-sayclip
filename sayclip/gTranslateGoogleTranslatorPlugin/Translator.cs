...
namespace gTranslateGoogleTranslatorPlugin
{
    [Export(typeof(sayclip.iSayclipPluginTranslator))]
    public class Translator : iSayclipPluginTranslator
    {
        private const string name = "gTranslate google translator plugin";
        private const string description = "Plugin using the translator from google translate (old API). Functionality provided by gTranslate lib https://github.com/d4n3436/GTranslate";
        ...
        public async Task<string> translate(string text)
        {
            ...
            try
            {
                ...
                translateResults = await googleTranslator.TranslateAsync(text, this.toLang, this.fromLang).ConfigureAwait(false);
                _context?.Logger?.Debug($"Detected results of google translator {translateResults}");
