...
namespace gTranslateGoogle2TranslatorPlugin
{
    [Export(typeof(sayclip.iSayclipPluginTranslator))]
    public class Translator : iSayclipPluginTranslator
    {
        private const string name = "gTranslate google2 translator plugin";
        private const string description = "Plugin using the translator from google translate (new RPC API). Functionality provided by gTranslate lib https://github.com/d4n3436/GTranslate";
        private string fromLang;
        private string toLang;
        private SayclipLanguage fromLangSayclip;
        private SayclipLanguage toLangSayclip;
        private GoogleTranslator2 google2Translator;
        ...
        public Task<IEnumerable<SayclipLanguage>> getAvailableLanguages(string displayLanguaje)
        {
            ...
            {
                if(kv.Value.IsServiceSupported(TranslationServices.Google))
                {
        ...
        public bool initialize(ISayclipPluginContext context = null)
        {
            ...
            this.google2Translator = new GoogleTranslator2();
        ...
        {
            GoogleTranslationResult translateResults;
            string result;
            try
            {
                _context?.Logger?.Debug($"translating {text} \n from {this.fromLang} to {this.toLang}");
                translateResults = await google2Translator.TranslateAsync(text, this.toLang, this.fromLang).ConfigureAwait(false);
                _context?.Logger?.Debug($"Detected results of google2 translator {translateResults}");
