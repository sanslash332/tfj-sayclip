...
namespace gTranslateMicrosoftTranslatorPlugin
{
    [Export(typeof(sayclip.iSayclipPluginTranslator))]
    public class Translator : iSayclipPluginTranslator
    {
        private const string name = "gTranslate microsoft translator plugin";
        private const string description = "Plugin using the translator from microsoft azure. Functionality provided by gTranslate lib https://github.com/d4n3436/GTranslate";
        ...
        public Task<IEnumerable<SayclipLanguage>> getAvailableLanguages(string displayLanguaje)
        {
            ...
            {
                if(kv.Value.IsServiceSupported(TranslationServices.Microsoft))
                {
        ...
        {
            MicrosoftTranslationResult translateResults;
            string result;
            try
            {
                _context?.Logger?.Debug($"translating {text} \n from {this.fromLang} to {this.toLang}");
                translateResults = await microsoftTranslator.TranslateAsync(text, this.toLang, this.fromLang).ConfigureAwait(false);
                _context?.Logger?.Debug($"Detected results of microsoft translator {translateResults}");
